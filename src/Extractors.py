...
class Trading212Client:
    def __init__(self):
        ...
        try:
            ...
            self.headers = {}
            logging.info(f"Failed to generate Trading212 headers, only demo version is available. {e}")
        ...
        self.output_folder = Path('/opt/airflow/raw_data/raw_trading_data') 
...
{quote_plus(self.pg_password)}@{self.pg_host}:{self.pg_port}/{self.pg_db}")
    ...
    def get_mock_positions(self):
        ...
        """
        prefix = "raw_positions_data"
        file_path = Path(f"/opt/airflow/mock_data/mock_positions.json")
    ...
    def get_mock_cash(self):
        ...
        """
        prefix = "raw_cash_data"
        file_path = Path(f"/opt/airflow/mock_data/mock_cash.json")
        raw_data = file_path.read_text(encoding='utf-8')
        data = json.loads(raw_data)
        self._save_json(data=data, prefix=prefix)
    ...
    def load_raw_json(self):
        ...
        try:
            ...
            logging.error(f"Required raw data files not found. Did you run API fetchers? {e}")
            raise
        except json.JSONDecodeError as e:
            logging.error(f"Error parsing JSON files: {e}")
            raise
        query = text(f"""
...
class NBPClient:
    def __init__(self):
        ...
        self.output_folder = Path('/opt/airflow/raw_data/raw_currency_data')
...
{quote_plus(self.pg_password)}@{self.pg_host}:{self.pg_port}/{self.pg_db}")
    ...
    def load_raw_json(self):
        ...
        try:
            ...
            logging.error(f"Error loading local currency JSON: {e}")
            raise
